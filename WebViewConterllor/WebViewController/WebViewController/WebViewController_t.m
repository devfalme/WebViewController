...
static char WebViewControllerKVOContext = 0;

@interface WebViewController_t ()
...
@implementation WebViewController_t
@synthesize URL = _URL;
...
- (void)commonInit {
    self.supportedWebNavigationTools = WebNavigationToolAll;
    self.supportedWebActions = SupportedWebActionAll;
    self.webNavigationPrompt = WebNavigationPromptAll;
    ...
    self.webView = [[WebView alloc] initWithFrame:self.view.bounds configuration:config];
    [self.webView.configuration.userContentController addUserScript:noneSelectScript];
    self.webView.backgroundColor = [UIColor whiteColor];
    self.webView.allowsBackForwardNavigationGestures = YES;
    self.webView.UIDelegate = self;
    self.webView.delegate_t = self;
    ...
    [self.webView addObserver:self forKeyPath:@"loading" options:NSKeyValueObservingOptionNew context:&WebViewControllerKVOContext];
...
- (UIBarButtonItem *)backwardBarItem {
    if (!_backwardBarItem) {
        ...
        _backwardBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Backward", @"WebViewLocalized", @"Accessibility label button title");
...
- (UIBarButtonItem *)forwardBarItem {
    if (!_forwardBarItem) {
        ...
        _forwardBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Forward", @"WebViewLocalized", @"Accessibility label button title");
...
- (UIBarButtonItem *)homeBarItem {
    if (!_homeBarItem) {
        ...
        _homeBarItem.landscapeImagePhone = nil;
        _homeBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Home", @"WebViewLocalized", @"Accessibility label button title");
        _homeBarItem.enabled = YES;
    }
    return _homeBarItem;
}
...
- (UIBarButtonItem *)actionBarItem {
    if (!_actionBarItem) {
        ...
        _actionBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Menu", @"WebViewLocalized", @"Accessibility label button title");
...
- (NSArray *)navigationToolItems {
    ...
    if ((self.supportedWebNavigationTools & WebNavigationToolBackward) > 0 || self.supportsAllNavigationTools) {
        [items addObject:self.backwardBarItem];
    }
    
    if ((self.supportedWebNavigationTools & WebNavigationToolForward) > 0 || self.supportsAllNavigationTools) {
        [items addObject:flexibleSpace];
        [items addObject:self.forwardBarItem];
    }
    
    if ((self.supportedWebNavigationTools & WebNavigationToolHome) > 0 || self.supportsAllNavigationTools) {
        [items addObject:flexibleSpace];
        [items addObject:self.homeBarItem];
    }
    
    if ((self.supportedWebNavigationTools & WebNavigationToolStopReload) > 0 || self.supportsAllNavigationTools) {
        [items addObject:flexibleSpace];
...
- (BOOL)supportsAllNavigationTools {
    return (_supportedWebNavigationTools == WebNavigationToolAll) ? YES : NO;
}

- (UIImage *)backwardButtonImage {
    if (!_backwardButtonImage) {
        _backwardButtonImage = [UIImage imageNamed:@"Left_t" inBundle:[NSBundle bundleForClass:[WebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)forwardButtonImage {
    if (!_forwardButtonImage) {
        _forwardButtonImage = [UIImage imageNamed:@"Right_t" inBundle:[NSBundle bundleForClass:[WebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)homeButtonImage {
    if (!_homeButtonImage) {
        _homeButtonImage = [UIImage imageNamed:@"Home_t" inBundle:[NSBundle bundleForClass:[WebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)reloadButtonImage {
    if (!_reloadButtonImage) {
        _reloadButtonImage = [UIImage imageNamed:@"Refresh_t" inBundle:[NSBundle bundleForClass:[WebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)stopButtonImage {
    if (!_stopButtonImage) {
        _stopButtonImage = [UIImage imageNamed:@"Stop_t" inBundle:[NSBundle bundleForClass:[WebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)actionButtonImage {
    if (!_actionButtonImage) {
        _actionButtonImage = [UIImage imageNamed:@"Menu_t" inBundle:[NSBundle bundleForClass:[WebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (NSArray *)applicationActivitiesForItem:(id)item {
    ...
    if ((_supportedWebActions & SupportedWebActionCopyLink) > 0 || self.supportsAllActions) {
        [activities addObject:[ActivityView activityWithType:ActivityTypeLink]];
    }
    if ((_supportedWebActions & SupportedWebActionOpenSafari) > 0 || self.supportsAllActions) {
        [activities addObject:[ActivityView activityWithType:ActivityTypeSafari]];
    }
    if ((_supportedWebActions & SupportedWebActionCleanCache) > 0 || self.supportsAllActions) {
        [activities addObject:[ActivityView activityWithType:ActivityTypeCleanCache]];
    }
...
- (NSArray *)excludedActivityTypesForItem:(id)item {
    ...
    if ((_supportedWebActions & SupportedWebActionshareLink) == 0) {
    ...
    if ((_supportedWebActions & SupportedWebActionReadLater) == 0 && [item isKindOfClass:[UIImage class]]) {
...
- (BOOL)supportsAllActions {
    return (_supportedWebActions == SupportedWebActionAll) ? YES : NO;
}
...
- (void)setTitle:(NSString *)title {
    if (self.webNavigationPrompt == WebNavigationPromptNone) {
...
- (void)setLoadingError:(NSError *)error {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedStringFromTable(@"Error", @"WebViewLocalized", nil) message:error.localizedDescription preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"OK", @"WebViewLocalized", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
- (BOOL)showNavigationPromptTitle {
    if ((self.webNavigationPrompt & WebNavigationPromptTitle) > 0 || self.webNavigationPrompt == WebNavigationPromptAll) {
        return YES;
    }
    return NO;
}
...
- (BOOL)showNavigationPromptURL {
    if ((self.webNavigationPrompt & WebNavigationPromptURL) > 0 || self.webNavigationPrompt == WebNavigationPromptAll) {
        return YES;
    }
    return NO;
}
...
- (void)showBackwardHistory:(UIGestureRecognizer *)sender {
    ...
    [self presentHistoryControllerForTool:WebNavigationToolBackward fromView:sender.view];
}
...
- (void)showForwardHistory:(UIGestureRecognizer *)sender {
    ...
    [self presentHistoryControllerForTool:WebNavigationToolForward fromView:sender.view];
}

- (void)presentHistoryControllerForTool:(WebNavigationTools)tool fromView:(UIView *)view {
    UITableViewController *controller = [UITableViewController new];
    controller.title = NSLocalizedStringFromTable(@"History", @"WebViewLocalized", nil);
...
- (void)configureToolBars {
    ...
    if (self.hideBarsWithGestures) {
        [self.navigationBar addObserver:self forKeyPath:@"hidden" options:NSKeyValueObservingOptionNew context:&WebViewControllerKVOContext];
        [self.navigationBar addObserver:self forKeyPath:@"center" options:NSKeyValueObservingOptionNew context:&WebViewControllerKVOContext];
        [self.navigationBar addObserver:self forKeyPath:@"alpha" options:NSKeyValueObservingOptionNew context:&WebViewControllerKVOContext];
    }
...
- (void)updateStateBarItem {
    ...
    self.stateBarItem.accessibilityLabel = NSLocalizedStringFromTable(self.webView.isLoading ? @"Stop" : @"Reload", @"WebViewLocalized", @"Accessibility label button title");
...
- (void)webView:(WebView *)webView didUpdateProgress:(CGFloat)progress {
...
- (WebView *)webView:(WebView *)webView createWebViewWithConfiguration:(WKWebViewConfiguration *)configuration forNavigationAction:(WKNavigationAction *)navigationAction windowFeatures:(WKWindowFeatures *)windowFeatures {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView.tag == WebNavigationToolBackward) {
        return self.webView.backForwardList.backList.count;
    }
    if (tableView.tag == WebNavigationToolForward) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (context != &WebViewControllerKVOContext) {
...
- (void)dealloc {
    if (self.hideBarsWithGestures) {
        [self.navigationBar removeObserver:self forKeyPath:@"hidden" context:&WebViewControllerKVOContext];
        [self.navigationBar removeObserver:self forKeyPath:@"center" context:&WebViewControllerKVOContext];
        [self.navigationBar removeObserver:self forKeyPath:@"alpha" context:&WebViewControllerKVOContext];
    }
    [self.webView removeObserver:self forKeyPath:@"loading" context:&WebViewControllerKVOContext];
